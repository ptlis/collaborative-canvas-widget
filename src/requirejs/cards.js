...
var cards = {
    ...
    init : function() {
        ...
        $('.zoom_in')
            ...
            .off(   'mouseenter').on(   'mouseenter',       cards.handlers.deckMouseEnter)
            .off(   'mouseleave').on(   'mouseleave',       cards.handlers.deckMouseLeave);
    ...
    addEvents : function(cardElem) {
        ...
        var deckHandler = cards.getDeckHandler(cardElem.data('carddeck'));
    ...
    getDeckHandler : function(deck) {
        'use strict';

        var deckHandler;

        // Valid deck
        if(deck in cards.availableDecks && cards.availableDecks.hasOwnProperty(deck)) {
            deckHandler = cards.availableDecks[deck];
        }
    ...
    addDeckHandler : function(deckName, deckObj) {
        'use strict';

        cards.availableDecks[deckName]  = deckObj;
    },


    removeDeckHandler : function(deckName) {
        'use strict';

        if(deckName in cards.availableDecks && cards.availableDecks.hasOwnProperty(deckName)) {
            delete  cards.availableDecks[deckName];
        }
    },
    ...
    propagateData : function(cardElem) {
        ...
        var extraData   = cards.getDeckHandler(cardElem.data('carddeck')).getExtraFields(cardElem.data('cardtype'));
            ...
            if(extraData.hasOwnProperty(index) && index in cardData) {
                ...
                // Push standard stored data into elems
                    ...
                    if(storageElem.prop('tagName').toLowerCase() === 'textarea' || (storageElem.prop('tagName').toLowerCase() === 'input' && (storageElem.attr('type') === 'text' || storageElem.attr('type') === 'hidden'))) {
                        ...
                        if(cardData[index] !== null && cardData[index].length > 0 && storageElem.attr('src') !== imgURL) {
    ...
    view : {
        add : function(event, cardData) {
            ...
            var deckHandler     = cards.getDeckHandler(cardData.deck);
        ...
        update : function(event, cardData) {
            ...
            var cardElems       = $('[data-instanceid="' + cardData.id + '"]');
            var cardElem;
            var extraData       = cards.getDeckHandler(cardData.deck).getExtraFields(cardData.cardtype);
    ...
    handlers : {
        ...
        paste : function(cellId, dropPosX, dropPosY) {
            ...
            if(cards.clipboardData.method === 'copy') {
                var instanceId          = uid_generator();
                var cardData            = cards.model.get(cards.clipboardData.instanceId);
                var cardExtraData       = cards.getDeckHandler(cardData.deck).getExtraFields(cardData.cardtype);
                var storeData           = {};
        ...
        changeActiveDecks : function() {
            ...
            decks.createSelectionDialog(false);
        },
        ...
        deckDragStart : function(event, dd) {
            'use strict';
            ...
            if($(this).hasClass('inactive')) {
                ...
                cards.handlers.deckMouseLeave(event);// Hide tooltip
        ...
        pairedElementDialog : function(event) {
            ...
            title.text(cards.availableDecks[deck].deckData.cardTypes[cardType].details.headings[headingIndex]);
            title.appendTo(dialog);
            ...
            var descElem;
            var cardData        = cards.model.get(instanceId);
            var cardExtraData   = cards.getDeckHandler(deck).getExtraFields(cardType);


            for(var itemIndex in cards.availableDecks[deck].deckData.cardTypes[cardType].details.items[headingIndex]) {
                if(cards.availableDecks[deck].deckData.cardTypes[cardType].details.items[headingIndex].hasOwnProperty(itemIndex)) {
                    ...
                    if(selected !== 'true') {
                        ...
                        titleElem           =   $('<em></em>');
                        titleElem.text(cards.availableDecks[deck].deckData.cardTypes[cardType].details.items[headingIndex][itemIndex].title);

                        descElem            =   $('<p></p>');
                        descElem.text(cards.availableDecks[deck].deckData.cardTypes[cardType].details.items[headingIndex][itemIndex].description);
